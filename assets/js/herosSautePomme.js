...
//action :  SAUTE (quand on appuie sur la flèche du haut)
var idIntervalSaut = 0;
  ...
  if (xxx === "ArrowUp") {
    if (!enTrainDeSauter) {
      if (direction == "devant") {
        ...
        idIntervalSaut = setInterval(function () {
          ...
          divHero.style.left = parseFloat(divHero.style.left) + 10 + "px";
          if (parseFloat(divHero.style.left) > 700) {
            decor.style.left = parseFloat(decor.style.left) - 10 + "px";
          }
          iSaut++;
          if (iSaut < 4) {
            divHero.style.top = parseFloat(divHero.style.top) - 60 + "px";
          } else {
            divHero.style.top = parseFloat(divHero.style.top) + 180 + "px";
          }
        ...
        idIntervalSaut = setInterval(function () {
          detecterCollisionPomme();
          divHero.style.width = actions.jumpGauche[iSaut].divHeros.width;
          divHero.style.height = actions.jumpGauche[iSaut].divHeros.height;
          hero.style.top = actions.jumpGauche[iSaut].heros.top;
          hero.style.left = actions.jumpGauche[iSaut].heros.left;
          divHero.style.left = parseFloat(divHero.style.left) - 10 + "px";
          if (parseFloat(divHero.style.left) > 700) {
            decor.style.left = parseFloat(decor.style.left) + 10 + "px";
          }
          ...
          if (iSaut === actions.jumpGauche.length) {
            clearInterval(idIntervalSaut);
            enTrainDeSauter = false;
            iSaut = 0;
          }
